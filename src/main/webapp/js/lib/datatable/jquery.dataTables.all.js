...
(/** @lends <global> */function( $ ) {
	...
	function _fnDraw( oSettings )
	{
		...
		else
		{
			...
			if(oSettings._iRecordsTotal > 0){
				sZero = '<span class="text-red">查询条件冲突,没有匹配的记录 * 请点击下方分页按钮进行查看</span>';
			}
	...
	function _fnReDraw( settings, holdPosition )
	{
		...
		if(!settings.oApi.isResetiDisplayStartZero){
	...
	function _fnAjaxParameters( settings )
	{
		...
		/**
		 * 戈志刚添加处理删除第二页最后一条信息时分页问题
		 */
//		var rowAry = api.rows();
//		if(rowAry[0].length == 1){
//			settings._iDisplayStart = displayStart = (displayStart == 0?displayStart:displayStart - 10);
//		}
	...
	function _fnAjaxUpdateDraw ( settings, json )
	{
		...
		if(settings._iDisplayStart >= settings._iDisplayLength && settings._iDisplayStart == settings._iRecordsTotal){
	...
	DataTable = function( options )
	{
		...
		this.fnDraw = function( complete )
		{
			if(complete == null) {
				...
				if (event) {
					/* 目前不支持火狐浏览器，火狐下无法获取 event事件*/
					var obj = event.srcElement ? event.srcElement : event.target;
					if (this.getObjectClass(obj) != null && this.getObjectClass(obj) != "XMLHttpRequest") {
						complete = true;
					} else {
						complete = false;
					}
...
var factory = function( $, DataTable ) {
	...
	DataTable.ext.renderer.pageButton.bootstrap = function ( settings, host, idx, buttons, page, pages ) {
		...
		var totalPage = settings._iDisplayLength === -1 ?
				0 : Math.ceil( settings.fnRecordsDisplay() / settings._iDisplayLength );
		var attach = function( container, buttons ) {
			var i, ien, node, button;
			var clickHandler = function ( e ) {
				e.preventDefault();
		...
		var parentDom = $(host).addClass('pagination').empty().html('<ul />').children('ul');

		parentDom.append('<li class="allpage"><span>共 <i>'+pages+'</i> 页 <i>'+settings._iRecordsTotal+'</i> 条记录 </span></li>');
		
		attach(parentDom,buttons);
...
$.extend( true, $.fn.dataTable.defaults, {
	...
	"sPaginationType": "full_numbers",//显示 共 0 页 0 条记录
	"oLanguage": {
	    "sProcessing":   "正在加载中...",
...
getTableParameters = function (table, aoData) {
	...
	var $page = $("#iPage");
	if($page.val() != null&& $page.val().length>0){
		 aoData.push({ "name": "page", "value": $("#iPage").val() });
		 curPage = $page.val(),$page.val("");
	}else{
