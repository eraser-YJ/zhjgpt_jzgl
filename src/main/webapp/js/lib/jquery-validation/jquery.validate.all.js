...
$.validator = function( options, form ) {
	...
	this.currentTable = $(this.currentForm).find('table');
	this.init();
};
...
$.extend($.validator, {
	...
	prototype: {
		...
		element: function( element ) {
			...
			this.showErrors();
			/**
			 * 解决IE8 错误信息消失后，页面不渲染问题，
			 * 添加一个样式，页面就从新review
			 * 只要是修改元素的cssText属性，不论它的值是什么，都会导致浏览器reflow和repaint
			 */
			if(this.currentTable.length){
				var tabledom = this.currentTable[0];
				tabledom.style.cssText = tabledom.style.cssText + ';zoom : 1;'
			}
			return result;
		},
...
var checkIdcard = function(idcard) {  
    ...
    //校验长度，类型  
    if(isCardNo_custom(card) === false) {
        return false;  
    }  
    //检查省份  
    if(checkProvince_custom(card) === false) { 
        return false;  
    }  
    //校验生日  
    if(checkBirthday_custom(card) === false) { 
        return false;  
    }  
    //检验位的检测  
    if(checkParity_custom(card) === false) { 
        return false;  
    }
    return true;  
}; 
...
//检查号码是否符合规范，包括长度，类型  
var isCardNo_custom = function(card)  
{  
...
var vcity_custom = { 11:"北京",12:"天津",13:"河北",14:"山西",15:"内蒙古",  
...
//取身份证前两位,校验省份  
var checkProvince_custom = function(card)  
{  
    var province = card.substr(0,2);  
    if(vcity_custom[province] == undefined)  
    {  
        return false;  
    }  
    return true;  
...
//检查生日是否正确  
var checkBirthday_custom = function(card)  
{  
    var len = card.length;  
    ...
    {  
        ...
        return verifyBirthday_custom('19'+year,month,day,birthday);  
    }  
    ...
    {  
        ...
        return verifyBirthday_custom(year,month,day,birthday);  
    }  
    return false;  
};  
  
//校验日期  
var verifyBirthday_custom = function(year,month,day,birthday)  
{  
    var now = new Date();  
...
//校验位的检测  
var checkParity_custom = function(card)  
{  
    //15位转18位  
    card = changeFivteenToEighteen_custom(card);  
    var len = card.length;  
    if(len == '18')  
    {  
...
//15位转18位身份证号  
var changeFivteenToEighteen_custom = function(card)  
{  
    if(card.length == '15')  
    {  
        var arrInt = new Array(7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2);   
...
//验证身份证函数
function checkIdcard_bak(idcard){
