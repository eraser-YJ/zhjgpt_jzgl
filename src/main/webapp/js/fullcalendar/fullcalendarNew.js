...
var defaults = {
	...
	titleFormat: {
		month:"yyyy 年 MMMM",
		week:"yyyy 年 MMM d[ yyyy]{ '&#8212;'[ MMM] d}",
		day:"yyyy 年 MMM d dddd "
	},
	columnFormat: {
		month: 'ddd',
		week: 'ddd M/d',
...
function Header(calendar, options) {
	...
	function render() {
		...
		if (sections) {
			if(options.isDefaultStyle=="1"){
				element  = $("<div style='width:100%'>");
				var buttonDiv = $("<div style='min-width:43%' class='btn-group fl' data-toggle='buttons'>");
				var buttonContent = $(options.buttonText.buttonContent);
			    buttonDiv.append(buttonContent);
				var tableDiv = $("<div style='float:left;width:172px'>");
				tableDiv.append($("<table class='fc-header' style='width:100%'/>")
					.append(
						$("<tr/>")
				...
				var searchDiv =  $("<div   class='input-append m-b-none fr' >");
				var searchContent = $(options.buttonText.searchContent);
				searchDiv.append(searchContent);
				element.append(buttonDiv);
				element.append(tableDiv);
				element.append(searchDiv);
            }else if(options.isDefaultStyle=="2"){
	...
	function setHeight(height) {
		...
		bodyFirstCells.each(function(i, _cell) {
			if (i < rowCnt) {
				...
				cell.find('> div').css('min-height',"65px");
			}
		});
	...
	function renderSlotSegs(segs, modifiedEventId) {
		...
		for (i=0; i<segCnt; i++) {
			...
			if (eventElement = seg.element) {
				...
				height = Math.max(0, seg.outerHeight - seg.vsides +2);//+2 解决了“日”形式展现的时候标题显示不全的问题  xuweiping 20160429
