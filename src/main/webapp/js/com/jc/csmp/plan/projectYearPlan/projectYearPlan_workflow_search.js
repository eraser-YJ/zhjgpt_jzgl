    ...
    //设置权限按钮
    {
        "mData": function (source) {
            ...
            if (source.instanceId) {
                buttonStr += "<a class='a-icon i-new' target='_blank' href='" + getRootPath() + "/workflowHistory/showHistory.action?instanceId=" + source.instanceId + "&definitionId=" + source.definitionId + "' >流程历史</a>";
            }
            return buttonStr;
...
projectPlanDoneList.oTableFnServerParams = function (aoData) {
    ...
    var search_planSeqno = $('#search_planSeqno').val();
    if (search_planSeqno.length > 0) {
        aoData.push({'name': 'planSeqno', 'value': search_planSeqno});
    }
    var search_planStatus = $('#search_planStatus').val();
    if (search_planStatus.length > 0) {
        aoData.push({'name': 'planStatus', 'value': search_planStatus});
    }
    aoData.push({'name': 'canSearch', 'value': "Y"});
};
