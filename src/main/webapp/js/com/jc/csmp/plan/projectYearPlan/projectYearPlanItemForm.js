var projectYearPlanItemJsForm = {};
projectYearPlanItemJsForm.callbackFun;
/**
 * 初始化
 * @param config {
 *     title： 标题
 ...
 * }
 */
projectYearPlanItemJsForm.showNew = function (dicCode, dicName, callback) {
    projectYearPlanItemJsForm.callbackFun = callback;
    ...
    projectYearPlanItemJsForm.operatorModal('show');
};

projectYearPlanItemJsForm.showEdit = function (data, callback) {
    projectYearPlanItemJsForm.operatorModal('show');
    projectYearPlanItemJsForm.callbackFun = callback;
    projectYearPlanItemJsForm.fillData(data);
    $('#entityFormTitle').html('编辑项目');
};


projectYearPlanItemJsForm.getRowData = function () {
    var rowData = {
...
projectYearPlanItemJsForm.fillData = function (rowData) {
 ...
 * @returns {boolean}
 */
projectYearPlanItemJsForm.formValidate = function () {
    if (!$("#entityForm").valid()) {
        return false;
    }
    return true;
 ...
 * 保存或修改方法
 */
projectYearPlanItemJsForm.saveOrModify = function () {
    if (projectYearPlanItemJsForm.formValidate()) {
        projectYearPlanItemJsForm.operatorModal('hide');
        projectYearPlanItemJsForm.callbackFun(projectYearPlanItemJsForm.getRowData());
        return;
    }
};
projectYearPlanItemJsForm.typeChangeSize = 4;

//类型是否选择
projectYearPlanItemJsForm.checkType = function () {
    for (var nowIndex = 1; nowIndex < projectYearPlanItemJsForm.typeChangeSize; nowIndex++) {
 ...
 * @param operator
 */
projectYearPlanItemJsForm.operatorModal = function (operator) {
...
$(document).ready(function () {
    ...
    $('#saveBtn').click(function () {
        projectYearPlanItemJsForm.saveOrModify();
    });
