...
projectRelationOrderJsList.renderTable = function () {
    if (projectRelationOrderJsList.oTable == null) {
        projectRelationOrderJsList.oTable =  $('#gridTable').dataTable( {
            ...
            "sAjaxSource": getRootPath() + "/project/relation/myList.action",
...
projectRelationOrderJsList.loadModuleForLook = function (id){
    $("#formModule").load(getRootPath() + "/project/relation/loadForm.action", null, function() {
        projectRelationOrderJsForm.init({title: '查看工程联系单', operator: 'look', id: id, callback: function(id){
            $.ajax({
                type: "GET", data: {id: id}, dataType: "json",
                url: getRootPath() + "/project/relation/read.action",
...
$(document).ready(function(){
    ...
    $("#batchDeleteBtn").click(function () { projectRelationOrderJsList.delete(0); });
});