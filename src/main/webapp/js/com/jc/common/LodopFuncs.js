...
function getLodop(oOBJECT, oEMBED) {
    ...
    try {
        ...
        if (needCLodop()) {
            ...
            if (!LODOP) {
                msgBox.confirm({
                    content: "尚未安装打印插件，是否下载安装？",
                    success: function(){
                        window.open(getRootPath() + "/js/com/jc/common/CLodop_Setup_for_Win32NT.exe");
                    }
                });
                return;
            } else {
...
var lodopFunction = {};
lodopFunction.print = function (option) {
    var top = option.top || 25;
    var left = option.left || 70;
    var width = option.width || 648;
    var height = option.height || 970;
    var LODOP = getLodop();
    LODOP.PRINT_INIT("");
    if (option.orient != undefined) {
        width = option.width || 970;
        height = option.height || 648;
        LODOP.SET_PRINT_PAGESIZE(option.orient, 0, 0, "");
        LODOP.SET_SHOW_MODE("LANDSCAPE_DEFROTATED",1);
    }
    LODOP.ADD_PRINT_HTM(top, left, width, height, option.content);
    LODOP.PREVIEW();
};

lodopFunction.createFile = function(config) {
    $.ajax({
        type: "GET", data: config, dataType: "json", async: false,
        url: getRootPath() + "/create/file/createFile.action",
