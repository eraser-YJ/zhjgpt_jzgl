...
ORYX.CONFIG.PLUGINS_CONFIG =				ACTIVITI.CONFIG.rootPath + "/process-editor/editor-app/plugins.xml";
...
ORYX.CONFIG.EVENT_EXECUTE_UNDO = 			"executeCommandsUndo";
				...
				if (this.node.ownerDocument) {
					...
					if (this.fitToElemId || this._textHasChanged){
						...
						textLines.each((function(textLine, index){
							...
							var textText = textLine.trim();
							var textChatLen = textText.length;

							textText = textChatLen > 8 ? (textText.substring(0 ,8) + '...') : textText;

							tspan.textContent = textText;
					...
					//源代码if (this.fitToElemId) {  修改文字大于4个字换行问题
					if (this.fitToElemId == 11) {
...
ORYX.Core.StencilSet.Property = Clazz.extend({
	...
	groupName: function(){
		return this._jsonProp.groupName;
	},

	hiddenRequired : function(){
		return this._jsonProp.hiddenRequired;
	},
...
/** @lends ORYX.Core.AbstractShape.prototype */
{
    ...
    toJSON: function(){
		...
		if(json.stencil.id !== 'StartNoneEvent' && json.stencil.id !== 'BPMNDiagram'){
			json.pointToMy = this.getPointToMyShapes().map(function(shape){
				return {
					resourceId: shape.resourceId
				};
			});
...
ORYX.Editor = {
	...
	_generateGUI: function() {
		...
		var $canvasContainer =jQuery("#canvasSection");

		$canvasContainer.append(canvasParent);

		var containerWidth = $canvasContainer.width() > 0 ? ($canvasContainer.width() - 30) : ORYX.CONFIG.CANVAS_WIDTH;
		...
		this.getCanvas().setSize({
			width	: (containerWidth > ORYX.CONFIG.CANVAS_WIDTH ? containerWidth : ORYX.CONFIG.CANVAS_WIDTH),
			height	: ORYX.CONFIG.CANVAS_HEIGHT
		});		
						
	},

	destroyEditorEvent : function(){
	...
	*/
	...
	updateSelection: function() {
		...
		//console.log(this._subSelection);
	...
	*/
	createShape: function(option) {
		...
		this.setDisplayName(newShapeObject , option.overrideid , sset.stencil(shapetype).idWithoutNs());
		...
		//添加默认的折线点
		this.setDockers(newShapeObject);
		// Add to the canvas
		...
		if(option.connectingType && option.connectedShape && !(newShapeObject instanceof ORYX.Core.Edge)) {
			...
			this.setDisplayName(con , option.overrideid , 'SequenceFlow');
		...
		if(newShapeObject instanceof ORYX.Core.Edge && option.connectedShape) {
			...
			var startObj = start.getDockedShape();
			var endObj   = end.getDockedShape();
	...
	getPositions: function(edge){
		...
		if(from && to){
			// Get absolute bounds
			var start = {
					x: from.bounds.a.x + (from.bounds.b.x - from.bounds.a.x) /2,
					y: from.bounds.a.y + (from.bounds.b.y - from.bounds.a.y) /2
				},
				end ={
					x: to.bounds.a.x + (to.bounds.b.x - to.bounds.a.x) /2,
					y: to.bounds.a.y + (to.bounds.b.y - to.bounds.a.y) /2
				};

			psition.a.x = (end.x - start.x) / 3 + start.x;
			psition.a.y = (end.y - start.y) / 3 + start.y;

			psition.b.x = (end.x - start.x) / 3 * 2 + start.x;
			psition.b.y = (end.y - start.y) / 3 * 2 + start.y;
		}

		return psition;
	},
	//添加默认显示折线拖拽节点
	setDockers: function(edge){
		if (!edge){ return }
		if(edge instanceof ORYX.Core.Edge){
			var _this = this;
			//添加延时,保证页面绘制完成后触发添加折线
			window.setTimeout(function(){
				var positions = _this.getPositions(edge);
				//删除所有拖拽节点
				edge.dockers.each(function(r){
					edge.removeDocker(r);
				});
				//从新添加拖拽节点
				[positions.a, positions.b].compact().each(function(pos){
	...
	setDisplayName : function(shape , ids , type){
		function getNumber(){
			return (ids[type] ? ids[type] : 0) + 1;
		}
		...
		if(shape.properties){
			var id 	= type.toLowerCase()+getNumber();
			var name= getName(type);

			shape.properties['oryx-overrideid'] = id;
			shape.properties['oryx-name'] = name;
			//字段属性
			shape.properties['oryx-field_attribute'] = ids.field;
		}
	},
...
*/
	...
	_handleMouseUp: function(event, uiObj) {
		...
		var elementController = uiObj;
		//console.log(canvas.node.getScreenCTM());
		//console.log(canvas.absoluteXY());
...
ORYX.Core.Shape = {
	...
	getPointToMyShapes: function(iterator) {
		if(iterator) {
			this.incoming.each(iterator);
		}
		return this.incoming;
	...
	setDockedShape: function(shape) {
		...
		if(this._dockedShape) {
			...
			if(this === this.parent.dockers.first()) {
				...
				this.parent.targets.push(shape);
...
ORYX.Core.Node = {
    ...
    construct: function(options, stencil, facade){
		...
		ORYX.Log.gang('ORYX.Core.Node实体类，stencil._jsonStencil可以获取节点信息');
 ...
 *  封装ztree方法
 *  @example
 *  var zTreeObj = new ORYX.Plugins.Ztree({
 ...
 */
ORYX.Plugins.Ztree = Clazz.extend({
	...
	_init: function() {
		...
		new Ajax.Request(opt.url, {
			...
			method: 'get',
			onSuccess: function(transport){
				this.treeData = this._paeseData(JSON.parse(transport.responseText));
				this._initZtree();
			}.bind(this),
			onFailure: (function(transport){
				ORYX.Log.error("获取树结构数据发生错误：" + transport);
			}).bind(this)
		});
	},
	_paeseData : function(data){
		var reslut = [];
		for(var i = 0,len = data.length;i < len;i++){
			var item = data[i],sikn = '';
			if(item.type == 'org'){
				item.iconSkin = 'org';
			}
			reslut.push(item);
	...
	_initZtree : function(){
		var opt = this.option;
		this.myTree = jQuery.fn.zTree.init(opt.element, this._getSettings(), this._getData());
		this.myTree.expandAll(true);
	},
	 ...
	 **/
	_getData: function() {
		return this.treeData.length?this.treeData:null;
	},
	...
	_getSettings: function() {
		...
		settings.check = opt.nodeCheck?(function(){
			return {
				chkboxType : {'Y': 's', 'N': 'ps'},
				radioType : "all",
				chkStyle : opt.single?'radio':'checkbox',
				enable : true,
	...
	/**
	 * 选中节点
	 * @parms ids String 节点id,多个[id,id]
	 **/
	setData: function(ids){
		...
		}else if(Object.prototype.toString.call(ids) === '[object String]'){
			addNS(ids);
		}
	},
	...
	/**
	 * 添加一个节点
	 * @parms parentId String 父节点id 如不传参则默认插入到根节点下
	 * @parms nodes    Object 要插入的节点对象
	 **/
	addNode: function(orgId, typeNode, nodes) {
		var treeObj = this.getZtreeObj();
		var pNode = treeObj.getNodeByParam('dataId', orgId, null);

		var myParentNode = null;
		if(pNode){
			if(Object.prototype.toString.call(pNode.children) === '[object Array]'){
				for(var i = 0; i < pNode.children.length;i++){
					var cNode = pNode.children[i];
					if(cNode.dataId == typeNode.id){
						myParentNode = cNode;
					}
				}
			}
			if(!myParentNode){
				...
				treeObj.addNodes(pNode ,nobj);

				var mypNode = treeObj.getNodeByParam('id', pNode.id, null);
				for(var i = 0; i < mypNode.children.length;i++){
					var itemNode = mypNode.children[i];
					if(itemNode.id == typeNode.id){
						myParentNode = itemNode;
					}
				}
			}
			nodes.pId = myParentNode.id;
			treeObj.addNodes(myParentNode ,nodes);
			treeObj.refresh();
		}
	},
	/**
	...
	removeNode : function(id){
		var treeObj = this.getZtreeObj();
		treeObj.removeNode(treeObj.getNodeByParam('id', id, null));
		treeObj.refresh();
	},
 ...
 * button and returning to the previous site in workflowHistory.
...
//		//原来是center.y - 15，这里是双击节点出来的textarea
//		textInput.style.top = (center.y - 25) + 'px';
...
ORYX.Plugins.DragDropResize = ORYX.Plugins.AbstractPlugin.extend({
	...
	construct: function(facade) {
		...
		this.currentShapes 		= [];			// Current selected Shapes		/当前选定的形状
		//this.pluginsData 		= [];			// Available Plugins	  		/可用插件
		this.toMoveShapes 		= [];			// Shapes there will be moved	/需要改动的图形节点
		this.distPoints 		= [];			// Distance Points for Snap on Grid
		this.isResizing 		= false;		// Flag: If there was currently resized	/是否有调整
		this.dragEnable 		= false;		// Flag: If Dragging is enabled			/启动拖拽
		this.dragIntialized 	= false;		// Flag: If the Dragging is initialized /初始化时拖拽
...
//				}
			...
			if(elements.length == 1 && elements[0].isResizable) {
				if(elements[0].getStencil().idWithoutNs() === 'SubProcess'){
...
ORYX.Plugins.SelectedRect = Clazz.extend({
	...
	construct: function(parentId) {
		...
		this.dashedArea = ORYX.Editor.graft("http://www.w3.org/2000/svg", this.node,
			['rect', {x: 0, y: 0,
				'id' : 'gezhigang1005',
				'stroke-width': 1, stroke: '#777777', fill: '#FDF8C9',
				'opacity': '.5',
...
ORYX.Plugins.DragDocker = Clazz.extend({
	...
	construct: function(facade) {
		...
		this.isValid 				= false;
		this.dockerStartNodeCache 	= undefined;
		this.dockerStartNodeStatus	= false;
	...
	dockerMoved: function(event) {
		...
		if (this.docker.parent) {
			if (this.isStartDocker || this.isEndDocker) {
				...
				if(!this.dockerStartNodeStatus) {
					this.dockerStartNodeStatus = true;
					this.dockerStartNodeCache = evPos;
				}
				var additionalIEZoom = 1;
	            if (!isNaN(screen.logicalXDPI) && !isNaN(screen.systemXDPI)) {
	                var ua = navigator.userAgent;
	 ...
	 * @param event
     */
	dockerMovedPiaffe: function(event){
		//
		ORYX.Log.gang('我想在这里实现恢复移动状态,下面的变量是缓存开始移动之前的位置信息');
		//this.facade.registerOnEvent({
		//	type 	: ORYX.CONFIG.EVENT_EXECUTE_UNDO
		//});
	},
	/**
