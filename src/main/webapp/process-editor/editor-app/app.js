...
activitiModeler
  ...
  .config(['$selectProvider', '$translateProvider', function ($selectProvider, $translateProvider) {
      ...
      $translateProvider.useStaticFilesLoader({
          prefix: '../process-editor/editor-app/i18n/',
          suffix: '.json'
      });
  ...
  .run(['$rootScope', '$timeout', '$modal', '$translate', '$location', '$window', '$http', '$q',
        function($rootScope, $timeout, $modal, $translate, $location, $window, $http, $q) {
              ...
              $rootScope.deployInfoData  = [];

              $rootScope.fieldData = null;

		      $rootScope.staticIncludeVersion = Date.now();
		      
		      $rootScope.todoCustomProperty = fieldCustomProperty.todoCustomProperty;
		      
		      var todoCustomPropertyArray = [];
		      
		      for (var key in fieldCustomProperty.todoCustomProperty) {
		    	  todoCustomPropertyArray.push(fieldCustomProperty.todoCustomProperty[key]);
		      }
		      
		      $rootScope.todoCustomPropertyArray = todoCustomPropertyArray;
            ...
            var drawCanvasFinished = true;//切换流程状态控制

            $rootScope.initCanvas = function(data){
                var isInitd = false;

                if($rootScope.editor) isInitd = true;

                $rootScope.editor = null;
                $rootScope.modelData = null;
                jQuery('.ORYX_Editor').remove();
                jQuery('.Oryx_SelectionFrame').remove();
                jQuery('.resizer_southeast').remove();
                jQuery('.resizer_northwest').remove();
            ...
            $rootScope.fetchModel = function(modelId) {
                if(!drawCanvasFinished) return false;

                drawCanvasFinished = false;

                var modelUrl = KISBPM.URL.getModel(modelId);
                $rootScope.updateModalId = modelId;
                $http({method: 'GET', url: modelUrl}).
                    success(function (data, status, headers, config) {
                        if(!data){
                            data = {"id":"","stencilset":{"url":"stencilsets/bpmn2.0/bpmn2.0.json","namespace":"http://b3mn.org/stencilset/bpmn2.0#"},"jsonStr":{"stencil":{"id":"BPMNDiagram"}}};
                        }else{
                            data.model = JSON.parse(data.jsonStr);
                        }
                        delete data.jsonStr;
                        data.modelId = data.id;
                        $rootScope.initCanvas(data);
                    }).
                    error(function (data, status, headers, config) {
                      console.log('加载模型的id错误： ' + modelId + ' ' + data);
                    });
            }
            /**
             *获取字段属性的数据
             * @param callback
             */
            $rootScope.getFieldData = function(){
                /**
                 * 解析字段属性的值
                 * 1. 没有操作privilege属性添加默认为readonly
                 * 2. 新获取到的值比较判断数据库是否有删减增加字段
                 */
                function parseFieldData(datas){
                    $A(datas).each(function(field){
                        if(!field.privilege){
                            field.privilege = 'readonly';
                        }
                        
                        //如果字段没有自定义属性，则默认放置一个空对象
                ...
                //查询当前流程所选择的表ID
                var stencil = $rootScope.editor.getCanvas();
                var form = stencil.properties['oryx-form_apply'];
                //根据表查询当前表下的所有字段
                if(form && form.id){
                    $http({
                        method: 'GET',
                        url: KISBPM.URL.getFields(form.id)
                    }).success(function (data, status, headers, config) {
                        if(data && data.length){
                            $rootScope.fieldData = parseFieldData(data);
                        }else{
                            $rootScope.fieldData = [];
                        }
                    });
            ...
            function initDeployInfo(){
                $http({
                    method: 'GET',
                    url: KISBPM.URL.getDeployInfo() +'?version=' + $rootScope.staticIncludeVersion
                }).success(function (data, status, headers, config) {
                    if(data) $rootScope.deployInfoData = data;
                });
            }
            ...
            function initScrollHandling() {
                ...
                canvasSection.scroll(function() {
                    ...
                    jQuery('.resizer_southeast').hide();
                    jQuery('.resizer_northwest').hide();
            ...
            $rootScope.$on('$includeContentLoaded', function (event) {
	            if (!$rootScope.editorInitialized) {
	                ...
	                jQuery(window).resize(function () {
                        ...
                        $rootScope.waitingViewStyle = {
                            "height" : totalAvailable+'px',
                            "width" :  canvas.width() - 30 +'px',
                        }
