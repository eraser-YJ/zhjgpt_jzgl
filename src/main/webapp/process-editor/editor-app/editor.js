/**
 * Created by gezhigang on 2016/6/8.
 */
if (!ORYX) {
    var ORYX = {};
}s
...
ORYX.Editor = {
    ...
    createShape: function(option) {
        ...
        this.setDisplayName(newShapeObject, option.overrideid, sset.stencil(shapetype).type());


        //还剩下下边判断拖拽时 一起出来的路由设置overrideid
        ...
        if (option.connectingType && option.connectedShape && !(newShapeObject instanceof ORYX.Core.Edge)) {
            ...
            this.setDisplayName(con, option.overrideid, 'edge');
    ...
    setDisplayName: function(shape, ids, type) {
        if (shape.properties && shape.properties['oryx-overrideid'] == '') {
            if (type == 'node') {
                shape.properties['oryx-overrideid'] = 'UserTask' + ids.node;
                shape.properties['oryx-name'] = '用户' + ids.node;
            } else {
                shape.properties['oryx-overrideid'] = 'sequenceFlow' + ids.edge;
                shape.properties['oryx-name'] = '路由' + ids.edge;
            }
        }
    },
