 ...
 * @Version 1.0
 */
public class ProjectDesignResourceServiceImpl extends ResourceServiceImpl implements IResourceService {
    private JdbcTemplate jdbcTemplate;

    public ProjectDesignResourceServiceImpl() {
    ...
    public PageManagerMapData query(PageManager pageParam, List<QueryDataParam> paramList) {
        return query(PAGE_URL, pageParam, paramList, ResourceEnums.pt_project_design);
    }
    ...
    public List<DetailData> getByBusinessId(String id) {
        return getByPk(ResourceEnums.pt_project_design, DETAIL_URL, id);
    }

    @Override
    public int getCountByProjectNumber(String projectNumber) {
        return selectCountByProject(jdbcTemplate, ResourceEnums.pt_project_design, projectNumber);
    }

    @Override
    public List<DetailData> detailDataList(String dataId) {
        return getByDataId(ResourceEnums.pt_project_design.toString(), IResourceService.DETAIL_URL, dataId);
    }

    @Override
    public Map<String, Object> detail(String dataId) {
        return getColumnContent(getByDataId(ResourceEnums.pt_project_design.toString(), IResourceService.DETAIL_URL, dataId), null);
    }
