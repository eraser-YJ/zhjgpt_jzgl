 ...
 * @Version 1.0
 */
public class ProjectZtbResourceServiceImpl extends ResourceServiceImpl implements IResourceService {
    private JdbcTemplate jdbcTemplate;

    public ProjectZtbResourceServiceImpl() {
    ...
    public PageManagerMapData query(PageManager pageParam, List<QueryDataParam> paramList) {
        return query(PAGE_URL, pageParam, paramList, ResourceEnums.pt_company_projects_ztb);
    }
    ...
    public List<DetailData> getByBusinessId(String id) {
        return getByPk(ResourceEnums.pt_company_projects_ztb, DETAIL_URL, id);
    }

    @Override
    public int getCountByProjectNumber(String projectNumber) {
        return selectCountByProject(jdbcTemplate, ResourceEnums.pt_company_projects_ztb, projectNumber);
    }

    @Override
    public List<DetailData> detailDataList(String dataId) {
        return getByDataId(ResourceEnums.pt_company_projects_ztb.toString(), IResourceService.DETAIL_URL, dataId);
    }

    @Override
    public Map<String, Object> detail(String dataId) {
        return getColumnContent(getByDataId(ResourceEnums.pt_company_projects_ztb.toString(), IResourceService.DETAIL_URL, dataId), null);
    }
