 ...
 * @Version 1.0
 */
public class ProjectApprovalResourceServiceImpl extends ResourceServiceImpl implements IResourceService {
    private JdbcTemplate jdbcTemplate;

    public ProjectApprovalResourceServiceImpl() {
    ...
    public PageManagerMapData query(PageManager pageParam, List<QueryDataParam> paramList) {
        ...
        paramList.add(QueryDataParam.create(ResourceOperatorTypeEnum.datetime.getType(), ResourceOperatorActionEnum.notnull.toString(), "approvalDate", null));
        return query(PAGE_URL, pageParam, paramList, ResourceEnums.pt_project_approval);
    }
    ...
    public List<DetailData> getByBusinessId(String id) {
        return getByPk(ResourceEnums.pt_project_approval, DETAIL_URL, id);
    }

    @Override
    public int getCountByProjectNumber(String projectNumber) {
        String sql = "select count(*) as cnum from " + getRealObjUrl(ResourceEnums.pt_project_approval) + " where " + ResourceEnums.pt_project_approval.getProject() + " = ? and approvalDate is not null";
