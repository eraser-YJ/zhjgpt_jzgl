...
/**
 * 安全问题信息为了统一接口。模拟资源的操作
 * @Author 常鹏
 * @Date 2020/7/27 14:44
 * @Version 1.0
 */
public class QuestionSafeResourceServiceImpl extends ResourceServiceImpl implements IResourceService {

    private JdbcTemplate jdbcTemplate;

    public QuestionSafeResourceServiceImpl() {
    ...
    public PageManagerMapData query(PageManager pageParam, List<QueryDataParam> paramList) {
        return query(PAGE_URL, pageParam, paramList, ResourceEnums.pt_project_safe);
    }
    ...
    public List<DetailData> getByBusinessId(String id) {
        return getByPk(ResourceEnums.pt_project_safe, DETAIL_URL, id);
    }

    @Override
    public int getCountByProjectNumber(String projectNumber) {
        return selectCountByProject(jdbcTemplate, ResourceEnums.pt_project_safe, projectNumber);
    }

    @Override
    public List<DetailData> detailDataList(String dataId) {
        return getByDataId(ResourceEnums.pt_project_safe.toString(), IResourceService.DETAIL_URL, dataId);
    }

    @Override
    public Map<String, Object> detail(String dataId) {
        return getColumnContent(getByDataId(ResourceEnums.pt_project_safe.toString(), IResourceService.DETAIL_URL, dataId), null);
    }
    ...
    public Result rsyncData(Object obj) throws Exception {
        ...
        param.put("objUrl", ResourceEnums.pt_project_safe.toString());
        ...
        if (result.isSuccess()) {
            ...
            ResourceEnums.pt_attach_info.getResourceService().rsyncData(ResourceAttachInfo.create(SpringContextHolder.getBean(IAttachService.class),
                    ...
                    entity.getId(), ResourceEnums.pt_project_safe.toString()));
        }
        return result;
    }
}
