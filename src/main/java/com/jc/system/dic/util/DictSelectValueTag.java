 ...
 * @date 2020-06-30
 */
public class DictSelectValueTag extends BaseDiyTag {
    ...
    public int doStartTag() throws JspException {
        ...
        sb.append("<select name='"+this.getName()+"' id='"+this.getId() + "' dictName='"+this.getDictName()
                + "' parentCode='"+this.getParentCode() +"' dataType='"+this.getType()+"'");
        generateAttribute(sb);
        sb.append(">");
        
        if(headName != null) {
            sb.append("<option value='").append(headName).append("' selected>")
                    .append(headName).append("</option>");
        }
        
        if(headType != null) {
        	if("1".equals(headType)) {
                sb.append("<option value='").append("全部").append("' selected>")
.append("-全部-").append("</option>");
            }
        	if("2".equals(headType)) {
                sb.append("<option value='").append("请选择").append("' selected>")
.append("-请选择-").append("</option>");
        ...
        for(BaseOption option : baseOptions) {
            sb.append("<option value='").append(option.getOptionName()).append("'");
            if(paramValue != null) {
                 if(paramValue.equals(option.getOptionName())) {
                     sb.append(" selected ");
                 }
            } else {
    ...
    public StringBuilder getChildList(StringBuilder childsb,String dictName,String parentCode) { 
        ...
        for(BaseOption option : baseOptions) {
        	childsb.append("<option value='").append(option.getOptionName()).append("'");
        	if(selectLinkage != null) {
                if(selectLinkage.equals(option.getOptionName())) {
