...
public class StdDateFormat extends DateFormat {
    ...
    protected transient DateFormat dateFormatRFC1123;
    protected transient DateFormat dateFormatISO8601;
    protected transient DateFormat dateFormatISO8601Z;
    protected transient DateFormat dateFormatPlain;
...
//          (String.format("Can not parse date \"%s\": not compatible with any of standard forms (%s)",dateStr, sb.toString()), pos.getErrorIndex());
                               ...
                               FieldPosition fieldPosition)
    {
        if (dateFormatISO8601 == null) {
            dateFormatISO8601 = (DateFormat) DATE_FORMAT_ISO8601.clone();
        }
        return dateFormatISO8601.format(date, toAppendTo, fieldPosition);
    }
    ...
    protected Date parseAsISO8601(String dateStr, ParsePosition pos)
    {
        ...
        if (len <= 10 && Character.isDigit(c)) {
           df = dateFormatPlain;
            if (df == null) {
                df = dateFormatPlain = (DateFormat) DATE_FORMAT_PLAIN.clone();
            }
        } else if (c == 'Z') {
            df = dateFormatISO8601Z;
            if (df == null) {
                df = dateFormatISO8601Z = (DateFormat) DATE_FORMAT_ISO8601_Z.clone();
            }
            ...
            if (hasTimeZone(dateStr)) {
                ...
                df = dateFormatISO8601;
                if (dateFormatISO8601 == null) {
                    df = dateFormatISO8601 = (DateFormat) DATE_FORMAT_ISO8601.clone();
                }
            } else {
    ...
    {
        if (dateFormatRFC1123 == null) {
            dateFormatRFC1123 = (DateFormat) DATE_FORMAT_RFC1123.clone();
        }
        return dateFormatRFC1123.parse(dateStr, pos);
    }
